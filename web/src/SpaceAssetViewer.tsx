...
export function SpaceAssetViewer({ gameId, playerId, roleId, leaseGeneration, onClose, onMessage }: { gameId: string; playerId: string; roleId: string; leaseGeneration: number; onClose: () => void; onMessage: (message: string) => void }) {
  ...
  return <section className="space-workspace">
    ...
    <div className={`space-body ${filterOpen ? "with-filters" : ""} ${selected !== undefined ? "with-detail" : ""}`}>
      ...
      {selected !== undefined && <aside className="space-detail"><button className="detail-close" onClick={() => { setSelected(undefined); setDetail(undefined); }}>×</button>{loadingDetail && <p>Loading card…</p>}{detail && <><div className="asset-title"><small>{detail.record.object_type} · {detail.record.orbital_regime.toUpperCase()}</small><h1>{detail.record.canonical_name}</h1><span>NORAD {selected} · {detail.record.cospar_id ?? "[REDACTED]"}</span></div><div className="detail-actions"><button onClick={() => selectedPosition.current && viewerRef.current?.camera.flyTo({ destination: selectedPosition.current })}>Focus</button><button className={follow ? "active" : ""} onClick={() => setFollow((value) => !value)}>Follow</button></div><section className="sensor-card"><h2>Public description</h2><p>{publicValue(detail.record.public_description)}</p><dl><div><dt>Operator</dt><dd>{publicValue(detail.record.operator)}</dd></div><div><dt>Mission</dt><dd>{publicValue(detail.record.mission_category)}</dd></div><div><dt>Status</dt><dd>{publicValue(detail.record.operational_status)}</dd></div></dl><h2>Sensors and payloads</h2>{detail.record.sensors?.length ? <ul>{detail.record.sensors.map((sensor) => <li key={sensor}>{sensor}</li>)}</ul> : <p className="redacted">[REDACTED]</p>}</section><section className="live-elements"><h2>Live pinned orbital data</h2>{["EPOCH", "APOAPSIS", "PERIAPSIS", "PERIOD", "INCLINATION", "ECCENTRICITY"].map((field) => <div key={field}><span>{field.replaceAll("_", " ")}</span><strong>{String(detail.raw_orbital_fields[field] ?? "[REDACTED]")}</strong></div>)}</section><section className="authority-card"><h2>Command authority</h2><strong>{detail.authority.display_name}</strong><p>{detail.authority.organization} · {detail.authority.kind.replaceAll("_", " ")} · {detail.authority.confidence}</p>{detail.authority.commandable ? <><textarea value={summary} maxLength={500} onChange={(event) => setSummary(event.target.value)} /><button className="command" onClick={() => void submitRequest("request_satellite_service")}>Request service</button><button className="secondary" onClick={() => void submitRequest("coordinate_satellite_maneuver")}>Coordinate maneuver</button></> : <p className="unresolved">Not commandable: reviewed public authority is unresolved or outside v1 scope.</p>}</section><Markdown markdown={detail.markdown} /><section><h2>Provenance</h2>{detail.sources.map((source) => <a href={source.url} target="_blank" rel="noreferrer" key={source.id}>{source.title} · {source.license}</a>)}</section></>}</aside>}
    </div>
...
export function publicValue(value: string | undefined) {
  return !value || value.trim() === "" || value.toLowerCase() === "unknown" ? "[REDACTED]" : value;
}
